...
def view_itinerary(trip_id: str) -> str:
    """Return a formatted plain-text itinerary for the given trip."""

    path = TRIPS_DIR / f"{trip_id}.json"
    if not path.exists():
        return f"No itinerary found for trip '{trip_id}'. Start planning with search_flights or search_hotels."
    ...
    if not items:
        return f"Itinerary for '{trip_id}' is empty. Add items with update_itinerary."
